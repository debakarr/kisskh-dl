...
class Mu38Extracter:      
    ...
    def _page_init_hook(self, page, m3u8_urls):
        ...
        client.on("Network.responseReceived", lambda response: self._append_mu38_urls(response, m3u8_urls))   
...
class Mu3u8Selector:
    ...
    def get_videos(self):
        ...
        for m3u8_playlist in m3u8_master.data["playlists"]:
            ...
            m3u8_master = m3u8.loads(response.text)
            m3u8_playlists[key] = [segment['uri'] for segment in m3u8_master.data['segments']]
            
        return m3u8_playlists
    
    
    
    def download_playlist_segments(self, outfile: Path | str, playlist_segments: list[str], number_of_workers: int=16):
        session = self._get_session()
        segments_dict = dict(zip(range(len(playlist_segments)), playlist_segments))
        q = queue.Queue()
        for segment in segments_dict.items(): q.put(segment)
        for _ in range(number_of_workers): q.put("")
        
        with tqdm(total=len(segments_dict), desc=f"Downloading content for {outfile}") as progress_bar:
            workers = []
        ...
        with open(outfile, "wb") as file:
            ...
            for i in tqdm(range(len(responses_dict)), desc=f"Writing content for {outfile}..."):
